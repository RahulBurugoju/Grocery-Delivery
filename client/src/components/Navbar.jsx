...
function Navbar() {
  ...
  return (
    <nav className="bg-white sticky top-0 z-50 border-b border-app-border">
      <div
        className="flex items-center justify-between gap-4 h-16 max-w-7xl mx-auto sm:px-4 lg:px-8 
            "
      >
        {/* Logo */}
        <Link
        ...
        <div className="w-full  flex items-center justify-end gap-4 lg:gap-10">
          ...
          {/* Sreach */}
          <form onSubmit={handleSearch} className="hidden sm:flex flex-1 mx-w-sm text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ShoppingCartIcon className="size-5 text-zince-900" />
              {cartCount > 0 && (
            ...
            <div className="relative">
              {user ? (
                <button
                  ...
                  className="flex items-cneter gap-2 p-2"
                >
                  <div className="size-7 rounded-full bg-green-950 text-white felx-center">
                    {user.name.charAt(0)}
                  </div>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                    ...
                    <div onClick={()=>setUserMenuOpen(false)}>
                        {!user && <Link to='/login' className="dropdown-link">
                        ...
                        {user?.isAdmin && (
                             ...
                             <ShieldIcon size={16}/><span className="text-app-orange-dark">Adim Panel</span>
                         </Link>
                        )}   

                        {user &&(
                        <div className="border-t border-app-border pt-1">
                            <button onClick={handelLogout} className="flex items-center gap-2 px-4 py-2.5 text-sm text-app-error hover:bg-red-50 w-full transition-colors">
