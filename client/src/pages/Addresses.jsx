...
function Addresses() {
  ...
  const [editingId, setEditinfgId] = useState(null);
  ...
  const onEditHandler = (addr)=>{
    setForm({label:addr.label,address:addr.address,city:addr.city,state:addr.state,zip:addr.zip,isDefault:addr.isDefault})
    setEditinfgId(addr._id);
    setShowForm(true)
  }
