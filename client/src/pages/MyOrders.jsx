...
function MyOrders() {
  ...
  useEffect(()=>{
    ...
    setLoading(false)
  },[activeTab, searchParams, setSearchParams, clearCart])
  return (
      ...
      <div className='max-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {orders.map((order)=>(
              <Link to={`/orders/${order._id}`} className='block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all'>
                  ...
                  <div className="">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-app-text-light">
                        {new Date(order.createAt).toLocaleDateString('en-Us',{month:"short", day:"numeric",year:"numeric"})}
                      </span>
                    </div>
                  </div>
                ...
                <div className='flex items-center gap-2 mb-3'>
                  ...
                  {order.items.length > 4 && (
                    <div className="size-2 sm:size-16 rounded-lg bg-app-cream flex-center text-xs font-semibold text-app-text-light">
                      +{order.items.length -4}
                    </div>
                  )}
                </div>
